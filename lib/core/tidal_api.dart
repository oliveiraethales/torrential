...
class TidalApi {
  ...
  Future<Map<String, dynamic>> _get(String path,
    ...
    if (response.statusCode == 401) {
      ...
      await auth.ensureValidToken();
      final retryResponse =
          await _httpClient.get(uri, headers: auth.apiHeaders);
  ...
  Future<List<Album>> getArtistAlbums(int artistId, {int limit = 50}) async {
    final data = await _get('/artists/$artistId/albums', params: {
      'limit': '$limit',
    });
  ...
  Future<List<Track>> getArtistTopTracks(int artistId,
      {int limit = 20}) async {
    final data = await _get('/artists/$artistId/toptracks', params: {
      'limit': '$limit',
    });
    final items = data['items'] as List<dynamic>;
  ...
  Future<List<Track>> getAlbumTracks(int albumId) async {
    final data = await _get('/albums/$albumId/tracks', params: {
      'limit': '100',
    });
    final items = data['items'] as List<dynamic>;
    return items.map((e) => Track.fromJson(e as Map<String, dynamic>)).toList();
  ...
  Future<AlbumCredits> getAlbumCredits(int albumId) async {
    final data = await _get('/albums/$albumId/credits');
    return AlbumCredits.fromJson(data);
  }
  ...
  Future<PlaybackInfo> getPlaybackInfo(int trackId,
      {AudioQuality quality = AudioQuality.hiResLossless}) async {
    final data = await _get('/tracks/$trackId/playbackinfopostpaywall', params: {
      'playbackmode': 'STREAM',
  ...
  Future<List<Track>> getPlaylistTracks(String uuid, {int limit = 100}) async {
    final data = await _get('/playlists/$uuid/tracks', params: {
      'limit': '$limit',
    });
    final items = data['items'] as List<dynamic>;
    return items.map((e) => Track.fromJson(e as Map<String, dynamic>)).toList();
  ...
  Future<List<Album>> getFavoriteAlbums({int limit = 100}) async {
    ...
    final data = await _get('/users/$userId/favorites/albums', params: {
      'limit': '$limit',
      'order': 'DATE',
      'orderDirection': 'DESC',
    });
  ...
  Future<List<Artist>> getFavoriteArtists({int limit = 100}) async {
    final userId = auth.userId;
    if (userId == null) throw Exception('User not logged in');
    final data = await _get('/users/$userId/favorites/artists', params: {
      'limit': '$limit',
      'order': 'DATE',
      'orderDirection': 'DESC',
    });
  ...
  Future<List<Track>> getFavoriteTracks({int limit = 100}) async {
    final userId = auth.userId;
    if (userId == null) throw Exception('User not logged in');
    final data = await _get('/users/$userId/favorites/tracks', params: {
      'limit': '$limit',
      'order': 'DATE',
      'orderDirection': 'DESC',
    });
  ...
  Future<List<Playlist>> getFavoritePlaylists({int limit = 100}) async {
    final userId = auth.userId;
    if (userId == null) throw Exception('User not logged in');
    final data = await _get('/users/$userId/favorites/playlists', params: {
      'limit': '$limit',
      'order': 'DATE',
      'orderDirection': 'DESC',
    });
  ...
  Future<List<Playlist>> getUserPlaylists({int limit = 100}) async {
    final userId = auth.userId;
    if (userId == null) throw Exception('User not logged in');
    final data = await _get('/users/$userId/playlists', params: {
      'limit': '$limit',
      'order': 'DATE',
      'orderDirection': 'DESC',
    });
  ...
  Future<void> addFavoriteTrack(int trackId) async {
    await _post('/users/${auth.userId}/favorites/tracks', body: {
      'trackId': '$trackId',
    });
  }

  Future<void> addFavoriteAlbum(int albumId) async {
    await _post('/users/${auth.userId}/favorites/albums', body: {
      'albumId': '$albumId',
    });
  }

  Future<void> addFavoriteArtist(int artistId) async {
    await _post('/users/${auth.userId}/favorites/artists', body: {
      'artistId': '$artistId',
    });
  }
